...
export interface AIChatContext {
  pageContext: string; // 'homepage' | 'analytics' | 'safety' | 'reports' | 'live-map' | 'sos' | 'hazards'
  locationName: string;
...
export type AISafetyLevel = 'CRITICAL' | 'WARNING' | 'ADVISORY' | 'NORMAL';
...
class AIServiceClass {
  /**
   * Sends a message to the AI Chatbot backend or processes locally with context grounding
   */
  async sendMessage(request: AIChatRequest): Promise<AIChatResponse> {
    ...
    try {
      const backendResponse = await ApiClient.post<AIChatResponse>('/ai/chat', {
        message: trimmed,
        context,
      });
      ...
      console.warn('[AIService] Backend API unavailable, utilizing contextual offline inference engine.', e);
    }

    // Local deterministic contextual response engine (Safety-verified & grounded)
    return this.generateGroundedResponse(trimmed, context);
  }

  /**
   * Generates a verified, context-grounded response using active telemetry and SOPs
   */
  private generateGroundedResponse(query: string, ctx: AIChatContext): AIChatResponse {
    const q = query.toLowerCase();
    const loc = ctx.locationName || 'your area';
    ...
    if (q.includes('current risk') || q.includes('how dangerous') || q.includes('risk score') || q.includes('risk level') || q.includes('my safety')) {
    ...
    if (q.includes('flood') || (ctx.selectedHazard?.toLowerCase().includes('flood') && (q.includes('do') || q.includes('guide') || q.includes('sop') || q.includes('what to do')))) {
      return {
    ...
    if (q.includes('cyclone') || q.includes('storm') || q.includes('wind') || ctx.selectedHazard?.toLowerCase().includes('cyclone')) {
      return {
    ...
    if (q.includes('earthquake') || q.includes('tremor') || q.includes('quak') || ctx.selectedHazard?.toLowerCase().includes('earthquake')) {
      return {
    ...
    // 9. What is happening near me (Page context aware)
    if (q.includes('near me') || q.includes('happening') || q.includes('around me') || q.includes('nearby')) {
      const nearbyList = ctx.nearbyHazards && ctx.nearbyHazards.length > 0
        ? ctx.nearbyHazards.map(h => `• **${h.title}** (${h.category.toUpperCase()}) - Severity: ${h.severity} (${h.distance || 'in vicinity'})`).join('\n')
        : `• **Flash Flood Advisory**: Lowland waterlogging monitored by CWC.\n• **Thunderstorm Alert**: Moderate lightning strikes detected by radar within 15 km.`;

      return {
        answer: `📍 **Live Situational Overview Around ${loc}** (Viewing ${ctx.pageContext.toUpperCase()}):\n\n${nearbyList}\n\n• **Composite Risk Score**: **${risk.score}/100 (${risk.level})**\n• **Weather Status**: ${weather.condition}, ${weather.temperature}°C, ${weather.humidity}% humidity.\n\nAll emergency services (NDRF, Fire & Police) are on active standby.`,
        sources: ['State Emergency Operation Centre', 'IMD Live Radar Grid'],
        safetyLevel: 'WARNING',
        suggestedActions: [
          { label: 'View on Live Map', actionTab: 'live-map', actionType: 'navigate' },
          { label: 'Hazard Analytics', actionTab: 'analytics', actionType: 'navigate' },
        ],
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      };
    }
