...
class SOSLocationUpdateRequest(BaseModel):
    ...
    heading_degrees: Optional[float] = Field(default=0.0)
...
class SOSResponderProfileRequest(BaseModel):
    ...
    vehicle_type: Optional[str] = Field(default="MOTORCYCLE")
    max_active_assignments: Optional[int] = Field(default=1)
...
async def get_sos_candidates(
    ...
    for c in candidates:
        ...
        enriched.append({
            ...
            "proximity_score": c.get("proximity_score", c.get("s_dist", 0.0)),
            "capability_score": c.get("capability_score", c.get("s_cap", 0.0)),
            "gps_freshness_score": c.get("gps_freshness_score", c.get("s_fresh", 0.0)),
            "workload_score": c.get("workload_score", c.get("s_work", 0.0)),
            "is_gps_fresh": c.get("is_gps_fresh", True),
            "gps_freshness_status": c.get("gps_freshness_status", "FRESH" if c.get("is_gps_fresh", True) else "STALE"),
            "gps_age_seconds": c.get("gps_age_seconds", 0.0),
...
async def update_responder_profile(
    ...
    if not pref:
        pref = UserPreference(
            ...
            capabilities=payload.capabilities or ["FIRST_AID"],
            vehicle_type=payload.vehicle_type or "MOTORCYCLE",
            max_active_assignments=payload.max_active_assignments or 1,
            heading_degrees=payload.heading_degrees,
            speed_kmh=payload.speed_kmh,
            gps_accuracy_m=payload.accuracy_meters,
        ...
        if payload.capabilities is not None:
            pref.capabilities = payload.capabilities
        if payload.vehicle_type is not None:
            pref.vehicle_type = payload.vehicle_type
        if payload.max_active_assignments is not None:
            pref.max_active_assignments = payload.max_active_assignments
        if payload.heading_degrees is not None:
            pref.heading_degrees = payload.heading_degrees
        if payload.speed_kmh is not None:
            pref.speed_kmh = payload.speed_kmh
    ...
    return ApiResponse(
        ...
        data={
            ...
            "capabilities": getattr(pref, "capabilities", []),
            "vehicle_type": getattr(pref, "vehicle_type", "MOTORCYCLE"),
            "max_active_assignments": getattr(pref, "max_active_assignments", 1),
...
async def accept_sos_offer(
    ...
    # 3. Compute Vehicle-Aware Turn-by-Turn Route & ETA
    vtype = getattr(pref, "vehicle_type", "MOTORCYCLE") if pref else "MOTORCYCLE"
    route_calc = await SOSRoutingEngine.calculate_route(
        from_lat=resp_lat,
        from_lon=resp_lon,
        to_lat=sos.latitude,
...
async def update_responder_location(
    ...
    pref = None
    if responder_id:
        p_res = await db.execute(select(UserPreference).where(UserPreference.user_id == responder_id))
    ...
    if assignment:
        ...
        assignment.speed_kmh = payload.speed_kmh or 0.0
        assignment.heading_degrees = payload.heading_degrees or 0.0
        assignment.is_stale_gps = False
        is_near_scene = SOSRoutingEngine.is_near_scene(assignment.distance_meters)
    ...
    # Real-time event across Citizen & Command Center streams
...
async def update_sos_status(
    ...
    # If transitioning to ON_SITE or RESOLVED, update active assignment
    now = utc_now()
    assign_res = await db.execute(
        select(SOSAssignment).where(SOSAssignment.sos_id == sos.id, SOSAssignment.status == "ACTIVE")
    )
    ...
    if payload.status.upper() == "ON_SITE" and assignment:
        assignment.arrived_on_site_at = now
    elif payload.status.upper() in ["RESOLVED", "CANCELLED"] and assignment:
        assignment.status = "COMPLETED" if payload.status.upper() == "RESOLVED" else "CANCELLED"
        assignment.completed_at = now
        if payload.status.upper() == "CANCELLED":
            assignment.cancelled_at = now
            assignment.cancellation_reason = payload.reason or "Incident cancelled"
    ...
    await NotificationService.broadcast_sos_event(
        event_name=event_name,
        sos=sos,
        extra_data={"arrived_on_site_at": assignment.arrived_on_site_at.isoformat() if assignment and assignment.arrived_on_site_at else None}
    )
